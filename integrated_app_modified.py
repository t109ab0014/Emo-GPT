...
def upload_file():
    ...
    if file and isinstance(file.filename, str):
        ...
        send_data_to_client(emotion_xyz)  # 更新數據
...
def send_data_to_client(emotion_xyz=None):
    data = generate_data(emotion_xyz)
    socketio.emit('update_data', data)

def update_data(emotion_xyz=None):
    send_data_to_client(emotion_xyz)  # 移除了 while True 循環
...
def generate_data(emotion_xyz=None):
    new_data = [emotion_xyz]
    global global_data
